...
`.claude/rules`와 `.claude/commands`가 `.agents/` 디렉토리를 가리키는 심볼릭 링크인지 확인합니다.
Git clone 시 자동 생성되지만, Windows 등 일부 환경에서 깨질 수 있습니다.

```bash
# 심볼릭 링크 검증 및 복구
if [ -L .claude/rules ] && [ -L .claude/commands ]; then
  echo "✅ 심볼릭 링크 정상: .claude/rules → .agents/rules"
else
  echo "⚠️ 심볼릭 링크 복구 중..."
  rm -rf .claude/rules .claude/commands 2>/dev/null
...
| 심볼릭 링크 | .claude/rules → .agents/rules | Claude Code 호환 |
