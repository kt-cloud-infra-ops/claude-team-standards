...
- **Project Rules** (`.claude/rules/` in repo) - Project-specific overrides
...
2. **Update All Affected Locations**
   ```
   Global change → ~/.claude/rules/ + .claude/rules/ + CLAUDE.md
   Project change → .claude/rules/<project>/ only
...
- Updated project override in .claude/rules/
