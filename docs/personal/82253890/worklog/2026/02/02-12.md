...
- **TECHIOPS26-100** E2E 테스트 assertion 품질 강화 — WEAK→STRONG 변환
  - 전체 테스트 assertion 감사: 26% STRONG / 74% WEAK (console.log만, expect 미사용)
  - incident-lifecycle.spec.ts: count/popup/DB 상태 검증 3건 추가, 중복 변수명 수정
  - maintenance-lifecycle.spec.ts: 활성/종료 상태, 그리드, 이력 검증 4건 추가
  - event-to-exception.spec.ts: console.log→expect() 7건 변환
  - person-in-charge.spec.ts: 검색 필드/그리드/팝업 검증 6건 추가
  - pagination-sort.spec.ts: 페이지네이션/정렬/행 수 검증 8건 추가
  - date-picker.spec.ts: 날짜 설정/초기화/검색 검증 7건 추가
...
- **O11y Mock 서버** 생성 (workspace/o11y-mock/mock_server.py)
  ...
  - test_mock.py 작성 → 42/42 테스트 통과
- **O11y system_code 통일** — ES0010 확정
  - implementation-guide.md OBS001→ES0010 3건 수정
  - 근거: 06-scheduler-api-migration.md (설계 확정문서)
- **O11y 통합테스트 DB 데이터 세팅**
  - docs/temp/o11y-test-data-setup.sql 작성 + 실행
  - inventory_master 4건, cmon_service_inventory_master 5건, x01_if_event_obs 15건
  - p_combine_event_obs 실행 → cmon_event_info 7건 생성 (infra 4 + platform 3)
  - Service 0건 (C1 prefix 매칭 버그 — 개발사 수정 대기)
  - host_group_nm → 실제 layer_nm 형식으로 수정 (권한 필터 통과 위해)
  - Platform용 cmon_layer_code_info layer 5 신규 2건 생성
  - G0001 그룹에 O11y layer 5배정 5건 추가
- **O11y 상세팝업 링크 수정** (subEventState.jsp)
  - O11y 이벤트 제목: dashboard_url 링크 추가 (source != zabbix일 때)
  - O11y 호스트명: dashboard_url 링크 추가
  - WAR 재빌드 완료

## 진행 중

- **E2E assertion 강화 후 전체 재실행 필요** — 서버 타임아웃으로 실행 실패 (267건 timeout), 서버 정상화 후 재확인
- **O11y 통합테스트 UI 검증** — 서버 기동 완료, 데이터 표시 여부 확인 중
  - DB 커넥션 풀 포화로 외부 psql 접속 불가 → 검증 미완
  - application-local.properties DB IP 변경 (10.2.14.54→192.168.128.11)
...
| docs/temp/o11y-test-data-setup.sql | 통합테스트 DB 데이터 세팅 SQL |
...
| workspace/o11y-mock/mock_server.py | O11y Mock 서버 |
| workspace/o11y-mock/test_mock.py | Mock 서버 API 계약 테스트 (42건) |

## 메모

- DB 커넥션 풀 이슈: luppiter_web HikariCP 3풀(main/itam/dcim) 기동 시 192.168.128.11 커넥션 포화 → 외부 psql 접속 불가
- application-local.properties DB IP를 192.168.128.11로 변경한 상태 (원래 10.2.14.54, VPN 미연결)
